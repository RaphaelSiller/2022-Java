...
public class KartentrickMethoden {
	...
	/**
	 * Wenn Spalte == 0 ist, also beim ersten Aufruf, wird ein 7*3 Array erstellt und mit den Zahlen von 1-21 gef�llt. 
	 * Es wird von oben nach unten und dann von rechts nach links inkrementierend aufgef�llt:
	 * [	
	 *  [1	 8	15]
	 * 	[2	 9	16]
	 * 	[3	10	17]
	 ...
	 * ]
	 * 
	 * Ansonsten werden alle Karten des �bergebenen Arrays spaltenweise "eingesammelt"
	 * und dann Zeilenweise wieder ausgegeben.
	 * @param karten, das Array, welches bearbeitet wird.
	 * @param spalte, in dem sich die vom Spieler ausgew�hlte Karte befindet. Die erste Spalte startet bei 1.
	 * @return
	 */
	public static int[][] fuellen(int[][] karten, int spalte) {
		 ...
		 * sorgt daf�r, dass man bei 1 anstelle von 0 startet. Spalte * 7 ist ein
		 ...
		 */
 		if(spalte == 0) { 
			...
			// Wenn die erste Spalte ausgew�hlt wurde, dann muss die dritte eingesammelt
			// werden
			spalte--; 
			if(spalte == 0)
				spalte = 3;
			...
			for (int j = 0; j < karten[k].length; j++) {
				...
				k = 0;
				// Ist f�r die Auswahl der n�chsten Spalte zust�ndig.
				// Wenn noch eine spalte frei ist, dann wird diese ausgew�hlt, ansonsten startet
	...
	/**
	 * Das �bergebene Array wird in die Konsole ausgegeben, wobei jedes Zeile ein int-Array darstellt.
