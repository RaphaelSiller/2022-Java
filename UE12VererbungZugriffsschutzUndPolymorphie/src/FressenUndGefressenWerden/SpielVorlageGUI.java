...
public class SpielVorlageGUI extends JFrame
{
	...
	private final int ANZAHL_GUTE_BEWEGLICHE_OBJEKTE = 5;
	private final int ANZAHL_BOESE_BEWEGLICHE_OBJEKTE = 3;
	/*
...
//			UnbeweglichesObjekt o = new UnbeweglichesObjekt("mauer.png");
//			contentPane.add(o);
//			int x = 0;
//			int y = 0;
//			do {
...
//		}
		...
		// Positioniert bewegliche gute Objekte im Fenster an zufällig ausgewählten Positionen
		// so dass diese sich nicht überdecken
		Sandwich.geschwindigkeit = 4;
		for (int i = 0; i < ANZAHL_GUTE_BEWEGLICHE_OBJEKTE; i++) {
			Sandwich o = new Sandwich();
			o.setxRichtung(1 + (int)(Math.random()*4));
			o.setyRichtung(1 + (int)(Math.random()*4));
			contentPane.add(o);
			int x = 0;
			int y = 0;
			do {
	...
	public void paint(Graphics g) {
		...
		if (komponenten != null && komponenten.length > 0) {
			for (int i = 0; i < komponenten.length; i = i + 1) {
				...
				if (!(u instanceof Mauer) && u.getObjektBei(u.getX() + richtung, u.getY()) == null)
					// Sollten die Objekte nicht kollidieren, werden sie in Richtung richtung
					// verschoben
					if(u instanceof Sandwich)
						((Sandwich) u).bewege();
					else
